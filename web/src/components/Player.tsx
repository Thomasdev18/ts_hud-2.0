    ...
    return (
        <div className={classes.wrapperPlayer}>
            <Group spacing={4} style={{ position: 'absolute', bottom: 5, ...getPositionStyle() }}>
                ...
                {oxygen < 100 && hudType === 'compact' && renderCompactProgress(oxygen, oxygen <= 20 ? 'red' : colors.oxygen, <TbLungsFilled size={16} />)}
                {oxygen < 100 && hudType === 'minimal' && renderMinimalProgress(oxygen, oxygen <= 20 ? 'red' : colors.oxygen, <TbLungsFilled size={16} />)}
